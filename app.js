...
const VOICEFLOW_RUNTIME_ENDPOINT =
  process.env.VOICEFLOW_RUNTIME_ENDPOINT || 'general-runtime.voiceflow.com'
          ...
          case 'choice': {
            ...
            if (buttons.length) {
              ...
              let filteredButtons = buttons
                .filter(
                  (buttons) => buttons.name != 'null' && buttons.name != null
                )
                .map(({ name, request }) => {
                  // Handle URL action
                  if (Object.keys(request.payload).includes('actions')) {
                    ...
                    if (request.payload?.actions?.length > 0) {
                      if (
                        Object.values(request.payload.actions[0]).includes(
                          'open_url'
                        )
                      ) {
        ...
        blocks: [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: cleanText(
                stripBackSlashs('Error. Please try again a bit later')
              ),
            },
          },
        ],
...
async function saveTranscript(username, userpix) {
  if (projectID) {
    ...
    if (!username || username == '' || username == undefined) {
      username = 'Anonymous'
      userpix =
        'https://avatars.slack-edge.com/2021-03-20/1879385687829_370801c602af840e43f8_192.png'
    }
    axios({
      method: 'put',
